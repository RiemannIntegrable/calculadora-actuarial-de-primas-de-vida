GrAmx <- function(tabla_mortalidad, x, r, i, m) {
    ### Validación de datos ###
    ...
    e <- ((1+i)/(1+r)) - 1 # rendimiento real
    ve <- 1/(1+e) #descuento real
    ...
    # Seguro de vida entero: Ax@e
    Axe <- sum(sapply(0:(omega-x), function(k) {
        ve^(k+1) * tpx[k+1] * qx_vec[k+1]
    }))

    # Seguro de vida entero con crecimiento geometrico.
    GrAx <- (Axe)/(1+r)
    
    ### Prima fraccionaria con valor asegurado creciente geometricamente asumiendo UDD ###
    GrAmx <- (i/im)*GrAx
